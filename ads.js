    ...
    constructor(twitchBlobUrl) {
      ...
      this.onmessage = function (e) {
        if (e.data.key == "ShowAdBlockBanner") {
          ...
          adBlockDiv.P.textContent = "Reklamlar Engelleniyor..";
        ...
        if (playerRootDiv != null) {
          ...
          if (adBlockDiv == null) {
            ...
            adBlockDiv.innerHTML =
              '<div class="player-adblock-notice" style="color: white; background: linear-gradient(135deg, #ff00cc, #333399); position: absolute; top: 0px; left: 0px; padding: 5px;"><p></p></div>';
            adBlockDiv.style.display = "none";
            ...
            if (qualityOverride) {
              ...
              if (quality == qualityOverride) {
                ...
                if (matchedResolutionUrl) {
                  console.log(
                    `qualityOverride(B) quality:${qualityOverrideFoundQuality} frameRate:${qualityOverrideFoundFrameRate}`
                  );
                } else {
                  console.log(
                    `qualityOverride(C) quality:${quality} frameRate:${frameRate}`
                  );
                }
                return matchedResolutionUrl
        ...
        try {
          ...
          if (playerType === "proxy")
            encodingsM3u8Response = await realFetch(
              "https://pxy.blocktwitchads.com/channel/" + CurrentChannelName
            );
          if (playerType === "proxy2") {
            encodingsM3u8Response = await realFetch(
              "https://api.ttv.lol/playlist/" +
                CurrentChannelName +
                ".m3u8%3Fallow_source%3Dtrue",
              {
                headers: {},
              }
            );
